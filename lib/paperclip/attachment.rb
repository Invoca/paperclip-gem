...
module Paperclip
  ...
  class Attachment
    ...
    #
    # RR Custom
    # Added to allow for intelligent "deep copy"
    #
    def clone_all attachment #:nodoc:
      log("[paperclip] cloning original #{'and all thumbnails ' unless ignore_additional_styles_on_copy}#{name}")

      styles = [:original]
      styles.concat(@options[:styles].keys) unless ignore_additional_styles_on_copy

      styles.each do |style|
        @queued_for_write[style] = attachment
      end
    end

    #
    # RR Override
    # => we are adding code to clone_all
    ...
    def assign uploaded_file
      ...
      if uploaded_file.is_a?(Paperclip::Storage::S3) && self.is_a?(Paperclip::Storage::S3) && uploaded_file.styles == self.styles
        # RR - our custom "deep copy" logic
        clone_all(uploaded_file) unless uploaded_file.original_filename.blank?
        ...
        [ :file_name,
          ...
          :width,
          :height,
          :file_size ].each do |attribute|
            instance_write(attribute, uploaded_file.instance_read(attribute, optional=true))
        end
        instance_write(:updated_at, Time.now)
        @dirty = true

      else
        # original untouched paperclip code
